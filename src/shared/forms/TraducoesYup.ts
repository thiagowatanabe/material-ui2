...
setLocale({
    ...
    string:{
        email: () => 'O campo precisa conter uma e-mail válido',
        max:({max}) => `O campo pode ter no máximo ${max} caracteres`,
        min:({min}) => `O campo pode ter pelos menos ${min} caracteres`,
        length:({length}) => `O campo pode ter ter exatemente ${length} caracteres`,
   },
   date: {
